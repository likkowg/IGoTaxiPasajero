...
export class HomePage {
  ...
  loadMap() {
    ...
    return this.geolocation.getCurrentPosition().then((resp) => {
      ...
      let directionService = new google.maps.DirectionsService(this.map);
          ...
          this.settingService.getPrices().subscribe(snapshot => {
            ...
            if (this.destination) {
              directionService.route({
                origin: new google.maps.LatLng(this.origin.location.lat, this.origin.location.lng),
                destination: new google.maps.LatLng(this.destination.location.lat, this.destination.location.lng),
                travelMode: 'DRIVING'
              }, (result) => {
